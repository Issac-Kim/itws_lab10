...
<?php
  ...
  @ $db = new mysqli('localhost', 'root', 'issachkim99', 'iit');
  ...
  if ($havePost) {
    ...
    $movies = htmlspecialchars(trim($_POST["title"]));  
    $year = htmlspecialchars(trim($_POST["year"]));
        
       $focusId = ''; // trap the first field that needs updating, better would be to save errors in an array
    
    if ($movies == '') {
      $errors .= '<li>Movie titles may not be blank</li>';
      if ($focusId == '') $focusId = '#movies';
    }
    if ($errors != '') {
      ...
      if ($dbOk) {
        ...
        $insQuery = "insert into actors (`title`,`year) values(?,?)";
        $statement = $db->prepare($insQuery);
        // bind our variables to the question marks
        $statement->bind_param("sss",$movieForDb,$yearForDb);
        // make it so:
        $statement->execute();
        
        // give the user some feedback
        echo '<div class="messages"><h4>Success: ' . $statement->affected_rows . ' movie added to database.</h4>';
        echo $title . ' made ' . $year . '</div>';
...
<form id="addForm" name="addForm" action="movies.php" method="post" onsubmit="return validate(this);">
  <fieldset> 
    <div class="formData">
      ...
      <label class="field" for="title">Movie:</label>
